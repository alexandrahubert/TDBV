...
- le jeu de données issu de Carhyce, intégrant des cours d’eau plus importants, de rang 1 à 4 sur l'HydroEcoRégion "Massif Armoricain" (n=`r n_sites_par_source[1,2]` stations). 
...
•	Les stations intégrant le jeu de données “Jan & Bossis” caractérisent naturellement les plus petits cours d’eau, marqués par une faible largeur (1 m en moyenne et toute inférieure à 5 m), une faible profondeur (moyenne de l’ordre de 30 cm) et, en conséquence, un faible rapport de forme (moyenne < à 8). Les bassins amont des stations échantillonnées sont petits (moyenne de l’ordre d’1 km²) ; toutefois, il est intéressant de remarquer que ce jeu de données présente une plus forte dispersion (hétérogénéité) de valeur de pente dont la moyenne est d’environ 2.2 % (0.022 m/m) et mais peut atteindre près de 9 % (0.09 m/m). 
•	Le jeu de données issus de la base de données Carhyce présente inversement une plus grande hétérogénéité des valeurs de géométrie du lit (largeur, hauteur). En effet, la largeur moyenne des stations est de l’ordre de 9 m et peut atteindre près de 30 m. de même la hauteur est en moyenne d’environ 90 cm mais peut dépasser 2 m. Cette hétérogénéité de géométrie se retrouve dans l’hétérogénéité de la taille du bassin versant amont puisque ceux-ci s’étendent sur des surfaces comprises entre 1 et plus de 800 km². Les valeurs de pente apparaissent inversement très homogènes sur ce sous-jeu de données avec des valeurs de pente très faible, de l’ordre 0,5% (0.005 m/m).
•	Le sous-jeu de données « Galineau » constitue un peu un « intermédiaire » entre les deux sous-jeu précédents. Les stations échantillonnées par Galineau sont marquées par une géométrie plus grande (largeur moyenne de l’ordre de 5 m et hauteur moyenne de 60 cm) mais également plus hétérogène que celles issues des travaux de Jan et Bossis. Comme pour le sous-jeu de données Carhyce, les stations renseignées par Galineau présentent des surface de bassin amont plutôt importantes, en moyenne de l’ordre de 30 km² mais marquées par une forte hétérogénéité. La pente moyenne de ce sous-jeu de données est de l’ordre de 1 % (0.01 m/m) mais reste assez hétérogène et peut atteindre 3 %.   
Ces analyses permettent de mettre en évidence les différences globales de géométrie des petits cours d’eau échantillonnés dans le cadre de 3 sous-jeux de données. Les données issues de « Jan et Bossis » se singularise par leur relative homogénéité des valeurs de gabarit du lit, qui apparaissent plus petites et moins larges que pour les autres sous-jeux de données. Ceci est parfaitement cohérent avec l’objectif de leur étude qui ciblait spécifiquement les cours d’eau de rang 1. Ces analyses suggèrent également des relations directes entre la surface de bassins amont, la pente et les caractéristiques géométrique du lit. Il apparaît en effet notamment que : 
...
Pour évaluer les effets combinés des deux variables explicatives (surface de bassin versant et pente), sur la largeur et la hauteur à plein bord des lits mineurs, des régressions multiples ont été réalisées. D'après les graphiques, les relations *log-log* semblent linéaires. Cependant, quelques essais montrent que les modèles sont mieux ajustés et ne transformant que les variables explicatives :
...
Afin d'obtenir le meilleur modèle sur les têtes de bassin versant, on l'ajuste sur le jeu de données combiné restreint aux bassins inférieurs à 10 km² (Brummer & Montgomery, 2003 ; MacDonald & Coe, 2007). Au-delà de cette valeur il est préférable de se reporter à Carhyce.
...
Tixier G, et al. 2012. Enjeux des têtes de bassins versants pour la préservation et la restauration du bon état des masses d’eau. Rapport d’étape ONEMA / LIEC.
