...
```{r}
# chargement du tableau de données au format RData
load(file = "../output/graphiques.RData")
```
...
Afin de le caler au mieux sur les têtes de bassins, on le cale sur le jeu de données combiné restreint aux bassins inférieurs à 20km². Au-delà de cette valeur il est préférable de se reporter à Carhyce.
...
output$pred_lpb <- renderText({ 
 raw_pred_lpb <-
    predict.lm(mf_lpb[[1]], newdata = new_data())
  
 paste0("Largeur plein bord : ", round(raw_pred_lpb, 2), "m")
})

output$pred_hpb <- renderText({ 
 raw_pred_hpb <-
    predict.lm(mf_hpb[[1]], newdata = new_data())
  
 paste0("Hauteur plein bord : ", round(raw_pred_hpb, 2), "m")
})
...
# essai value Box
# output$pred_lpb2 <- flexdashboard::renderValueBox({
#   
#  raw_pred_lpb2 <-
#     predict.lm(mf_lpb[[1]], newdata = new_data())
#   
#  flexdashboard::valueBox(value = as.character(raw_pred_lpb2),
#                          caption = "Largeur plein bord (m)",
#                          icon = "fa-thumbs-down")
#   })
# 
# flexdashboard::valueBoxOutput(outputId = "pred_lpb2")
# 
