...
author: "Julien Boëton & Pascal Irz"
date: "22/07/2022"
output:
...
```{r, fig.cap="Relation entre la pente de la ligne d'eau et la surgace du bassin versant, distinguant les trois jeux de données."}
g_pente_sbv
```
...
```{r, fig.cap="Distribution des valeurs du rapport entre largeur plein bord et hauteur plein bord pour chacun des trois jeux de données. Les lignes verticales indiquent la valeur moyenne pour chaque jeu de données."}
g_densite_lh
```

Le rapport largeur sur hauteur n'est lié nettement ni à la pente ni à la surface du bassin versant (graphiques non prsentés). Il apparaît en revanche que la valeur moyenne de ce rapport est inférieure dans le jeu de données "têtes de bassin" en comparaison des deux autres jeu.
...
Pour évaluer les effets combinés des deux variables explicatives, sur la largeur et la hauteur plein bord, des régressions multiples ont été réalisées. 
...
modeles %>% 
  rename(`Surface BV` = `Surface_BV_km2`,
         `Pente` = pente_eau_m_m,
          Constante = `(Intercept)`) %>% 
...
La superficie du bassin versant, avec des coefficients toujours significatifs, est clairement la principale variable qui contribue à expliquer lla largeur et la hauteur du lit. Ces coefficients sont positifs, indiquant un clair effet taille : plus un bassin versant est grand, plus le cours d'eau est large et profond.
...
Ces modèles sont très cohérents. La perte sur le coefficient de détermination ajusté est négligeable. Ces modèles semblent dont être les plus intéressants. 
